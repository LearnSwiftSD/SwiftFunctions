...
func incrament() -> (Int) -> Int {
    var aValue: Int = 0
    func theIncramenter(aNumber: Int) -> Int {
        aValue = aValue + aNumber
        return aValue
    }
    return theIncramenter
}

let anIncramenter = incrament()
 ...
 */
let anotherIncramenter: (Int) -> Int = {
    var aValue = $0 + $1
    return {
        aValue = aValue + $0
        return aValue
...
anotherIncramenter(1)
anotherIncramenter(1)
anotherIncramenter(3)
anotherIncramenter(4)
/*:
 ___
...
class ExampleClass {
    ...
    lazy var printer: ((Int) -> ()) = { [weak self] in
        self?.startNumber = $0
        print(self?.startNumber ?? 0)
    }
...
Array(0..<1_000).forEach {
    let aPrintingClass = ExampleClass()
        aPrintingClass.printer($0)
}
/*:
...
func getsCalledIn(seconds: TimeInterval, _ aFunction: @escaping () -> Void) {
    DispatchQueue.global().asyncAfter(deadline: .now() + seconds) {
        aFunction()
    }
}

getsCalledIn(seconds: 12) {
    print("Hello, from the past!")
}

print("I'm getting called last!")
...
let myExtractedNumber = userCache.value(forKey: "My teeth hurt", defaultValue: 0)
/*:
 ___
 [Home](Welcome) | [Previous](@previous) | [Next](@next)
 */
